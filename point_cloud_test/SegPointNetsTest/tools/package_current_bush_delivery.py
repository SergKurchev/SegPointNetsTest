...
def _copy_item(item: CopyItem, destination_root: Path) -> dict[str, object]:
    ...
    source_label = (
        f"point_cloud_test/{item.source.relative_to(_point_cloud_root()).as_posix()}"
        if item.source.is_relative_to(_point_cloud_root())
        else f"SegPointNetsTest/{item.source.relative_to(_seg_repo_root()).as_posix()}"
    )
    return {
        "source": source_label,
...
def _rewrite_text_paths(destination_root: Path) -> None:
    replacements = {
        r".": ".",
        r".\SegPointNetsTest": r".\SegPointNetsTest",
        r"..\strawberry_tracking": r"..\strawberry_tracking",
        r"..\Perception_In_Robotics\FP\strawberry_orbit_data_new": r"..\Perception_In_Robotics\FP\strawberry_orbit_data_new",
        r"..\Perception_In_Robotics\FP\strawberry_orbit_data_new\data": r"..\Perception_In_Robotics\FP\strawberry_orbit_data_new\data",
        "src/point_cloud_test/scripts/": "src/point_cloud_test/scripts/",
        "SegPointNetsTest/": "SegPointNetsTest/",
        "": "",
        "SegPointNetsTest/": "SegPointNetsTest/",
    }
    text_suffixes = {".md", ".py", ".ps1", ".toml", ".json"}
...
def _build_instructions_markdown() -> str:
    return """# Instructions
...
def _package_items() -> list[CopyItem]:
    ...
    return [
        CopyItem(point_root / "README.md", Path("README.md")),
        CopyItem(point_root / "MODEL_EVAL_GUIDE.md", Path("MODEL_EVAL_GUIDE.md")),
        CopyItem(point_root / "pyproject.toml", Path("pyproject.toml")),
        CopyItem(point_root / "src", Path("src")),
        CopyItem(seg_root / "README.md", Path("SegPointNetsTest/README.md")),
        CopyItem(seg_root / "tools" / "prepare_strawberry_benchmark.py", Path("SegPointNetsTest/tools/prepare_strawberry_benchmark.py")),
        CopyItem(seg_root / "tools" / "evaluate_strawberry_predictions.py", Path("SegPointNetsTest/tools/evaluate_strawberry_predictions.py")),
        CopyItem(seg_root / "tools" / "collect_pointcept_predictions.py", Path("SegPointNetsTest/tools/collect_pointcept_predictions.py")),
        CopyItem(seg_root / "tools" / "export_oneformer3d_predictions.py", Path("SegPointNetsTest/tools/export_oneformer3d_predictions.py")),
        CopyItem(seg_root / "tools" / "package_current_bush_delivery.py", Path("SegPointNetsTest/tools/package_current_bush_delivery.py")),
        CopyItem(seg_root / "scripts", Path("SegPointNetsTest/scripts")),
        CopyItem(seg_root / "configs" / "strawberry", Path("SegPointNetsTest/configs/strawberry")),
        CopyItem(point_root / "artifacts" / "dataset", Path("artifacts/dataset")),
        CopyItem(point_root / "artifacts" / "proof", Path("artifacts/proof")),
        CopyItem(point_root / "artifacts" / "benchmark_current_bush", Path("artifacts/benchmark_current_bush")),
        CopyItem(point_root / "artifacts" / "benchmark" / "metrics_overview.png", Path("artifacts/benchmark/metrics_overview.png")),
        CopyItem(point_root / "artifacts" / "benchmark" / "runtime_comparison.png", Path("artifacts/benchmark/runtime_comparison.png")),
        CopyItem(point_root / "artifacts" / "benchmark" / "sample_0001_qualitative.png", Path("artifacts/benchmark/sample_0001_qualitative.png")),
        CopyItem(point_root / "artifacts" / "reid" / "strawberry_remap.json", Path("artifacts/reid/strawberry_remap.json")),
        CopyItem(seg_root / "data_exports" / "strawberry_benchmark", Path("SegPointNetsTest/data_exports/strawberry_benchmark")),
        CopyItem(seg_root / "artifacts" / "predictions", Path("SegPointNetsTest/artifacts/predictions")),
        CopyItem(seg_root / "artifacts" / "reports", Path("SegPointNetsTest/artifacts/reports")),
    ]
...
def main() -> None:
    ...
    summary_markdown = _build_summary_markdown(benchmark_summary, benchmark_manifest, clustering_summary)
    (output_dir / "SUMMARY.md").write_text(summary_markdown, encoding="utf-8")
    (output_dir / "INSTRUCTIONS.md").write_text(_build_instructions_markdown(), encoding="utf-8")
    _rewrite_text_paths(output_dir)
